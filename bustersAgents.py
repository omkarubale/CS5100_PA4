...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        # initialize comparison distances
        distance = None
        distance2 = None
        ...
        # for each ghost's distribution
        for d in livingGhostPositionDistributions:
            # get ghost position
            position = d.argMax()
            # get maze distance to ghost
            mazeDist = self.distancer.getDistance(pacmanPosition, position)

            # minimize distance
            if (distance is None) or (mazeDist <= distance):
                # update distance to lowest
                distance = mazeDist
        ...
        for a in legal:
            # get successor position after action a
            successorPosition = Actions.getSuccessor(pacmanPosition, a)
            # get maze distance to closest ghost
            mazeDist2 = self.distancer.getDistance(successorPosition, ghostPos)

            # minimize distance2
            if (distance2 is None) or (mazeDist2 <= distance2):
                # update distance to lowest
                distance2 = mazeDist2
                # update to best action
                bestAction = a
