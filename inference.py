...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            previous_prob = self.beliefs[oldPos]
            # get new probability distibution based on previous position
            new_position_distro = self.getPositionDistribution(
                gameState, oldPos)

            # for each probability p
            for p in new_position_distro.keys():
                # update belief
                new_beliefs[p] += new_position_distro[p]*previous_prob
